 ...
 * Funciona correctamente con submenús de varios niveles de anidación.
 *
 */
 ...
 class bootstrap_5_wp_nav_menu_walker extends Walker_Nav_Menu {
    ...
    */
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        // Detectar si el elemento es del selector de idiomas (WPML)
        $is_wpml_item = in_array('wpml-ls-item', $classes);
        ...
        // Gestión especial para WPML: eliminar contenido HTML en `title`
        if ($is_wpml_item) {
            $atts['class'] .= ' wpml-selector';
            $atts['title'] = ''; // Eliminar el contenido del atributo `title` generado automáticamente
        } else {
            $atts['title'] = ! empty( $item->title ) ? strip_tags($item->title) : ''; // Limpiar HTML en otros casos
        }
        ...
        // Si es un elemento WPML, mostrar solo la bandera y el texto limpio
        if ($is_wpml_item) {
